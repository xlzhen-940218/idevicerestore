	...
	if (ipsw->zip) {
		...
		for (i = zstat.size; i > 0; i -= count) {
			...
			if (count < 0) {
				...
				zip_error_t* zip_error = zip_file_get_error(zfile);
				error("ERROR: zip_fread: %s %d %d\n", infile, zip_error->zip_err, zip_error->sys_err);
				ret = -1;
				break;
			}
...
static int ipsw_list_contents_recurse(ipsw_archive_t ipsw, const char* path, ipsw_list_cb cb, void* ctx)
{
	...
	while (ret >= 0) {
		...
		struct stat st;
#ifdef WIN32
		ret = _stat64(fpath, &st);
#else
		ret = l_stat64(fpath, &st);
#endif
		if (ret != 0) {
